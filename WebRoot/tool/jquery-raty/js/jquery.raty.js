...
(function($) {
	...
	$.fn.raty = function(settings) {
		...
		for (var i = 1; i <= options.number; i++) {
			...
			$this
			.append('<img id="' + containerId + '_' + i + '" src="' + path + starFile + '" alt="' + i + '" title="' + hint + '" class="' + containerId + '"/>')
			.append((i < options.number) ? '' : '');
		}
		$this.append('<input id="' + containerId + '_score" type="hidden" name="' + options.scoreName + '"/>');
			// Avoids a nonexistent index (undefined) and ensures that the hint will be applied, it means different from null. Otherwise applies the current number.
		$this.append('<span id="rateword" style="margin_left:10px;" ></span>');

		$('#' + containerId + '_score').val(start);

		if (showHalf) {
			var score = $('input#' + containerId + '_score').val(),																	// [x ... x.25] || [x.26 ... x.75] || [x.76 ... (x + 1)]
			...
			if (diff >= 0.3 && diff <= 0.7) {																						// 3.5			In the center interval the score is with a next star.
				...
				$('img#' + containerId + '_' + Math.ceil(rounded)).attr('src', path + starHalf);									// 				The next star is the half star.
				...
				$('img#' + containerId + '_' + rounded).attr('src', path + starOn);
			}
		}

		if (!options.readOnly) {
			if (options.showCancel) {
				var cancel = '<img src="' + path + options.cancelOff + '" alt="x" title="' + options.cancelHint + '" class="button_cancel"/>'; 
				...
				.live('click', function() {
					$('input#' + containerId + '_score').val(0);
					if (onClick) { 
			          onClick(0);
			        }
				});
			...
			.live('mouseenter', function() {
				...
				for (var i = 1; i <= qtyStar; i++) {
					if (i <= this.alt) {
						$('img#' + containerId + '_' + i).attr('src', path + starOn);
					} else {
						$('img#' + containerId + '_' + i).attr('src', path + starOff);
					}
				}
			})
			.live('click', function() {
				$('input#' + containerId + '_score').val(this.alt);
				var hintList2 = [ '在打酱油', '状态不佳', '工作顺利', '表现不错', '状态神勇' ];
				$("#rateword").replaceWith('<span id="rateword" style="margin-left:10px;" >'+hintList2[this.alt-1]+'</span>'); 
				if (onClick) { 
		          onClick(this.alt);
		        }
			});
			...
			$this.live('mouseleave', function() {
				var id = $(this).attr('id'), qtyStar = $('img.' + id).length, score = $('input#' + id + '_score').val();

				for (var i = 1; i <= qtyStar; i++) {
					if (i <= score) {
						$('img#' + id + '_' + i).attr('src', path + starOn);
					} else {
						$('img#' + id + '_' + i).attr('src', path + starOff);
					}
				}

				if (showHalf) {
					var score = $('input#' + id + '_score').val(), rounded = Math.ceil(score), diff = (rounded - score).toFixed(1);

					if (diff >= 0.3 && diff <= 0.7) {
						rounded = rounded - 0.5;
						$('img#' + id + '_' + Math.ceil(rounded)).attr('src', path + starHalf);
					} else if (diff >= 0.8) {
						rounded--;
					} else {
						$('img#' + id + '_' + rounded).attr('src', path + starOn);
					}
				}
			}).css('cursor', 'pointer');
		} else {
	...
	$.fn.raty.defaults = {
		...
		cancelPlace:	'left',
		hintList : [ '在打酱油', '状态不佳', '工作顺利', '表现不错', '状态神勇' ],
		number : 5,
		path : '../img/',
		readOnly:		false,
		scoreName:		'score',
	...
	function liveEnter() {
		...
		$('img.' + id).live('mouseenter', function() {
			...
			for (var i = 1; i <= qtyStar; i++) {
				if (i <= this.alt) {
					$('img#' + id + '_' + i).attr('src', options.path + options.starOn);
				} else {
					$('img#' + id + '_' + i).attr('src', options.path + options.starOff);
				}
			}
		});
	};
